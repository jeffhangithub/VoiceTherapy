...
## v1.9.12（2026-09-14）修 STT 误删真实发言 + 重新平衡「降噪 vs 识别准确率」

- **症状**：Jeff 反馈——为了降噪，识别准确率下降太严重。
- **根因（日志实证）**：`_looks_like_noise` 把「含 ≥3 连续拉丁字母」当噪声 → **含 ADHD / AI / PPT 等缩写的整句真话被丢**（2026-09-14 日志：丢弃 `对，因为我刚才说我自己是ADHD。`、`我知道我自己是ADHD，所以。` 等 4+ 句真实发言）；「<0.7s 却出多字即丢」又误删短句真话。
- **修**：噪声判定改为**只在「完全不含中文」时生效**（含任一汉字 = 真实发言，绝不丢）；**删掉「短音频即丢」**。降噪职责改由文本过滤器承担。
- **VAD 放宽**（两 orchestrator）：`confidence 0.6→0.55`、`start_secs 0.35→0.25`、`stop_secs 0.6→0.5`、`min_volume 0.5→0.4`（此前收紧导致截断/漏掉短句）。
- 变更：`sensevoice_stt.py`、`orchestrator.py`、`orchestrator_webrtc.py`。

## v1.9.11（2026-09-11）语音轨骨架改「人味版」+ 用 09-10 原话重放验证

- **问题**（重放暴露）：刚性骨架（S0–S5 清单式）虽修好了"无结构"，但**显刚性/盘问**：安全问反复、几乎每轮以问句收尾、开场丢连续性。
- **改**：骨架改为**"姿态 + 内部走向 + 几条硬线"**（明写"别报阶段、别赶进度"；"允许整条只是接住、不提问、不推进"；安全"自然关切一次、不反复追问"；开场"一句轻带过"）。`_PERSONA` 与骨架去重。
- **重放验证**（09-10 原话 37 段，flash）：安全询问 4→2；"我理解偏了"式口头禅减少；语气更暖（收尾带状态检查）。**但两处没动**：① 开场仍未回顾（虽已注入 recap 指令）② 结尾提问 28→29/37 —— **prompt 已到瓶颈**（问句结尾是模型强倾向；骨架"轻带过"被"今天什么状态"走向挤掉）。
- 变更：`counselor/SKILL.md`（v1.6.0）、`voice_orchestrator/counselor_context.py`。
