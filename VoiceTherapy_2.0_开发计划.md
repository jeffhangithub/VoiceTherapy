...
## 7.5 延迟优化（共性方案，已先行于 1.x 网页验证）

延迟大头（ASR 位置、LLM 首 token、TTS 首 chunk）**与浏览器/原生无关，属共性**。详见同仓库 `VoiceTherapy_响应延迟优化.md`。原生 App 直接继承，并因 native 音频栈可更进一步：

- **P1 轻语音 persona + 短首句**：原生照搬；native 端还可按需只带人设、去工具上下文。
- **P2 TTS 首句即播**：原生音频栈更易做到首字出声（前瞻 TTS / 低首 chunk 延迟）。
- **P3 流式 ASR**：原生可在**手机本地流式**（SenseVoice/whisper.cpp），边说边出中间转写 + **原始音频不出设备**（隐私）。
- **P4 网络**：原生同样依赖低 RTT；免持场景需注意蜂窝中继延迟。

目标：与 1.x 共用同一套优化结论，2.0 落地时不重复论证。
